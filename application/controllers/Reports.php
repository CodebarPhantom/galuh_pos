...
class Reports extends CI_Controller
{
    ...
    public function monthly_report_category()
    {
        $siteSettingData = $this->Constant_model->getDataOneColumn('site_setting', 'id', '1');
        $siteSetting_dateformat = $siteSettingData[0]->datetime_format;
        $siteSetting_currency = $siteSettingData[0]->currency;
        ...
        $data['lang_month'] = $this->lang->line('month');
        $data['lang_year'] = $this->lang->line('year');
        $data['lang_service_center'] = $this->lang->line('service_center');
        $data['lang_list_service'] = $this->lang->line('list_service');

        $this->load->view('monthly_report_category', $data);
    }
                ...
                if ($category_tenant=="all"){
                 ...
                 if ($category_tenant=="all"){
                    $rItemResult = $this->db->query("SELECT r.id, r.order_id, r.product_code, r.product_name,r.cost, r.price, r.qty, o.discount_percentage,c.name as tenant,c.id, p.promotion_name
                                  ...
                                  LEFT JOIN promotion as p on p.id = o.promo_id
                                  WHERE r.order_id = '$order_id'  ORDER BY r.id ");
                 }
                   else{
                    $rItemResult = $this->db->query("SELECT r.id, r.order_id, r.product_code, r.product_name,r.cost, r.price, r.qty, o.discount_percentage,c.name as tenant,c.id, p.promotion_name
                                FROM return_items as r 
                                ...
                                LEFT JOIN category as c ON r.product_category = c.id
                                LEFT JOIN promotion as p on p.id = o.promo_id
                                WHERE r.order_id = '$order_id' AND c.id = '$category_tenant' ORDER BY r.id ");
                 }
    ...
    public function exportMonthlyreportcategory()
    {
        $report = $this->input->get('report');
        $url_start = $this->input->get('start_date');
        $url_end = $this->input->get('end_date');
