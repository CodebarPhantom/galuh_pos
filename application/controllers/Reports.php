...
class Reports extends CI_Controller
{
    ...
    {  
        ...
        $category_tenant= strip_tags($this->input->post('category'));
         ...
         $objPHPExcel->getActiveSheet()->setCellValue('F2', "$lang_price");
         ...
         /*$orderResult = $this->db->query("SELECT o.id, o.customer_id,o.customer_name,o.customer_email,o.customer_mobile,o.ordered_datetime,o.outlet_id,o.outlet_name,o.outlet_address,o.outlet_contact,o.outlet_receipt_footer,
                    ...
                    o.refund_status,o.remark,o.card_number, sum(oi.price) as harga, oi.qty as qty
                    FROM orders  as o 
        ...
        if ($category_tenant=="all"){
            $orderResult = $this->db->query("SELECT o.id, o.customer_id,o.customer_name,o.customer_email,o.customer_mobile,o.ordered_datetime,o.outlet_id,o.outlet_name,o.outlet_address,o.outlet_contact,o.outlet_receipt_footer,      o.gift_card,o.subtotal,o.discount_total,o.discount_percentage,o.promo_id,o.tax,o.grandtotal,o.total_items,o.payment_method,o.payment_method_name,o.cheque_number,o.paid_amt,
                        o.return_change,o.created_user_id,o.created_datetime,o.updated_user_id,o.updated_datetime,o.vt_status,o.status,
                        o.refund_status,o.remark,o.card_number, sum(oi.price) as harga, oi.qty as qty
                        FROM orders  as o 
         ...
         for ($d = 0; $d < count($orderData); ++$d) {
             ...
             if ($order_type == '1') {                // Order;
                ...
                if ($category_tenant=="all"){
                    $oItemResult = $this->db->query("SELECT o.id, o.order_id, o.product_code, o.product_name,o.cost, o.price, o.qty, c.name as tenant, c.id
                    FROM order_items as o LEFT JOIN category as c ON o.product_category = c.id
                    WHERE order_id = '$order_id'  ORDER BY o.id ");
                }else{
                    $oItemResult = $this->db->query("SELECT o.id, o.order_id, o.product_code, o.product_name,o.cost, o.price, o.qty, c.name as tenant, c.id
                    FROM order_items as o LEFT JOIN category as c ON o.product_category = c.id
                    WHERE order_id = '$order_id' AND c.id = '$category_tenant' ORDER BY o.id ");
                }
                 ...
                 if ($oItemRows > 0) {
                     ...
                     for ($t = 0; $t < count($oItemData); ++$t) {
                         ...
                         $oItem_subtotal = $oItemData[$t]->price * $oItemData[$t]->qty;
                 ...
                 FROM return_items as r LEFT JOIN category as c ON r.product_category = c.id
             ...
             if (count($priceArray) > 0) {
                ...
                $objPHPExcel->getActiveSheet()->setCellValue("F$jj", "$f_price");
            } else {
            ...
            if (count($subtotalArray) > 0) {
                ...
                $objPHPExcel->getActiveSheet()->setCellValue("G$jj", "$f_subtotal");
            } else {
            ...
            if (count($taxArray) > 0) {
                $f_tax = '';
                $f_tax = $taxArray[0];
                $objPHPExcel->getActiveSheet()->setCellValue("H$jj", "$f_tax");
            } else {
            ...
            if (count($grandtotalArray) > 0) {
                ...
                $objPHPExcel->getActiveSheet()->setCellValue("I$jj", "$f_grandtotal");
            } else {
             ...
             if (count($pcodeArray) > 1) {
                 for ($p = 1; $p < count($pcodeArray); ++$p) {
                     ...
                     $objPHPExcel->getActiveSheet()->setCellValue("F$jj", "$s_price");
                     $objPHPExcel->getActiveSheet()->setCellValue("G$jj", "$s_subtotal");
                     $objPHPExcel->getActiveSheet()->setCellValue("H$jj", "$s_tax");
                     $objPHPExcel->getActiveSheet()->setCellValue("I$jj", "$s_grandtotal");
        ...
        // $margin20 = $total_sub_amt *0.2;
         ...
         $objPHPExcel->getActiveSheet()->setCellValue("A$kk", "$lang_margin_20");
         $objPHPExcel->getActiveSheet()->SetCellValue('G'.$kk, "=SUM(G3:G".($kk-2).")*0.2");        
         ...
         $objPHPExcel->getActiveSheet()->setCellValue("A$ll", "$lang_margin_80"); 
         $objPHPExcel->getActiveSheet()->SetCellValue('G'.$ll, "=SUM(G3:G".($ll-3).")*0.8");         
